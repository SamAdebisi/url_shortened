...
class ShortenerTest(TestCase):
    ...
    def test_retrieve_original_url(self):
        url = reverse('original_url', args=[self.shortened_url.shortened_url])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, self.original_url)

    def test_delete_shortened_url(self):
        url = reverse('delete_shortened_url', args=[self.shortened_url.shortened_url])
        response = self.client.delete(url)
        self.assertEqual(response.status_code, 204)
        self.assertFalse(ShortenedURL.objects.filter(id=self.shortened_url.shortened_url).exists())
    ...
    def test_shorten_url_caching(self):
        url = reverse('shorten')
        response1 = self.client.post(url, {'original_url': 'https://www.google.com/'})
        self.assertEqual(response1.status_code, 201)
        shortened_url = response1.data['shortened_url']

        # Clear cache and send request again
        cache.clear()
        response2 = self.client.post(url, {'original_url': 'https://www.google.com/'})
    ...
    def test_authentication(self):
        ...
        response = self.api_client.post(url, {'original_url': 'https://www.google.com/'})
        self.assertEqual(response.status_code, 401)
